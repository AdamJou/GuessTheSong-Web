...
export const useSessionStore = defineStore("session", {
    state: () => ({
        ...
        _roomStatusUnsub: null,
        _gameSubUnsub: null,
        _roundSubUnsub: null,
        _playersSubUnsub: null,
    }),
    actions: {
        initializeSession() {
        ...
        subscribeToRoundStatus() {
            if (!this.roomId || !this.currentGame || !this.currentRound)
                return;
            const db = getDatabase();
            const roundStatusRef = dbRef(db, `rooms/${this.roomId}/games/${this.currentGame}/rounds/${this.currentRound}/status`);
        ...
        async setRoundStatus(status) {
            if (!this.roomId || !this.currentGame || !this.currentRound)
                return;
            const db = getDatabase();
            const roundRef = dbRef(db, `rooms/${this.roomId}/games/${this.currentGame}/rounds/${this.currentRound}`);
